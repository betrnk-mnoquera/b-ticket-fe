...
import { brochures } from '@/data/mockData'

const stats = [
  { icon: 'menu_book', label: 'Total Brochures', value: '32' },
  { icon: 'check_circle', label: 'Published', value: '18' },
  { icon: 'visibility', label: 'Total Views', value: '12.4K' },
  { icon: 'description', label: 'Avg. Pages', value: '6' },
]
...
export default function Brochures() {
  ...
  const filtered = filter === 'All' ? brochures : brochures.filter(b => b.status === filter.toLowerCase().replace(/\s+/g, '_'))
  ...
  if (selected) {
    return (
      <div>
        ...
        <PageHeader breadcrumbs={[{ label: 'Dashboard', href: '/' }, { label: 'Brochures', href: '#' }, { label: selected.name }]} title="" />
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          <StatsCard icon="visibility" label="Total Views" value="1,247" />
          <StatsCard icon="download" label="Downloads" value="89" />
          ...
          <div className="col-span-2 space-y-4">
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {[['Brochure ID', selected.brochureId], ['Store', selected.store], ['Pages', selected.pages], ['File Size', '2.4 MB'], ['Dimensions', 'A4 (210×297mm)'], ['Created', selected.createdAt], ['Updated', '2025-07-10']].map(([l, v], i) => (
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="space-y-2 mb-3">
                <div className="flex items-center justify-between p-2.5 bg-muted rounded-lg">
                  <div className="flex items-center gap-2"><div className="w-8 h-8 rounded bg-red-100 flex items-center justify-center text-xs font-bold text-red-600">PDF</div><div><div className="text-sm font-medium">{selected.name}.pdf</div><div className="text-xs text-muted-foreground">2.4 MB</div></div></div>
          ...
          <div className="space-y-4">
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div><div className="text-sm font-medium">{selected.store}</div><div className="text-xs text-muted-foreground">Manila, PH</div></div>
              </div>
            </div>
            <div className="bg-card border border-border rounded-xl p-5">
        ...
        <DeleteModal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={() => { setDeleteTarget(null); setSelected(null); toast('Brochure deleted') }} entityName={deleteTarget?.name} />
      </div>
    )
  }
    ...
    <div>
      ...
      <div className="bg-card border border-border rounded-xl shadow-[0_2px_4px_rgba(0,0,0,0.04)]">
              ...
              <div key={b.id} className="border border-border rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1"><Icon name="visibility" size={12} /> {b.views.toLocaleString()}</span>
                    <span className="flex items-center gap-1"><Icon name="calendar_today" size={12} /> {b.createdAt}</span>
                  </div>
                  <div className="flex items-center justify-between mt-3">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-muted flex items-center gap-1"><Icon name="store" size={10} /> {b.store}</span>
            ...
            <tbody>
              {filtered.map(b => (
                <tr key={b.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  <td className="px-4 py-3"><div className="flex items-center gap-2.5"><div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center"><Icon name="menu_book" size={16} className="text-primary" /></div><div><div className="font-medium">{b.name}</div><div className="text-xs text-muted-foreground">{b.brochureId}</div></div></div></td>
        ...
        <div className="px-4 py-3 border-t border-border">
          <Pagination currentPage={page} totalPages={4} totalItems={32} itemsPerPage={8} onPageChange={setPage} />
        </div>
      </div>

      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="Add Brochure"
        footer={<>
          ...
          <button onClick={() => { setDrawerOpen(false); toast('Brochure created') }} className="px-4 py-2 text-sm font-medium rounded-full bg-primary text-white hover:opacity-90">Save</button>
        </>}
      >
        <div className="space-y-4">
          ...
          <FormField label="Status" select options={['Published', 'Draft', 'Archived']} />
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Published From" type="date" />
            <FormField label="Published Until" type="date" />
          </div>
      ...
      <DeleteModal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={() => { setDeleteTarget(null); toast('Brochure deleted') }} entityName={deleteTarget?.name} />
    </div>
  )
}
