...
import { lineOfBusiness } from '@/data/mockData'

const stats = [
  { icon: 'category', label: 'Total Line of Business', value: '12' },
  { icon: 'store', label: 'Total Stores', value: '486' },
  { icon: 'trending_up', label: 'Most Popular', value: 'Food' },
  { icon: 'analytics', label: 'Avg. Stores/Category', value: '40' },
]
...
export default function LineOfBusiness() {
  ...
  const filtered = filter === 'All' ? lineOfBusiness : lineOfBusiness.filter(l => l.status === filter.toLowerCase())
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<button onClick={() => { setDrawerOpen(true); setFields([{ name: '', type: 'text', options: '' }]); setSelectedIcon('restaurant') }} className="px-4 py-2 text-sm font-medium rounded-full bg-primary text-white hover:opacity-90 flex items-center gap-1.5"><Icon name="add" size={16} /> Add Category</button>}
      />
          ...
          <tbody>
            {filtered.map(cat => (
              <tr key={cat.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                ...
                <td className="px-4 py-3 font-medium">{cat.stores}</td>
                <td className="px-4 py-3">
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="Add Category" width="w-[560px]"
        footer={<><button onClick={() => setDrawerOpen(false)} className="px-4 py-2 text-sm font-medium rounded-full border border-border hover:bg-muted">Cancel</button><button onClick={() => { setDrawerOpen(false); toast('Category created') }} className="px-4 py-2 text-sm font-medium rounded-full bg-primary text-white hover:opacity-90">Save</button></>}
      >
        <div className="space-y-5">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Category Name</label>
            <input className="w-full px-3 py-2 text-sm rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Description</label>
            <textarea className="w-full px-3 py-2 text-sm rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-ring h-20 resize-none" />
          </div>
      ...
      <DeleteModal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={() => { setDeleteTarget(null); toast('Category deleted') }} entityName={deleteTarget?.name} />
    </div>
  )
}
